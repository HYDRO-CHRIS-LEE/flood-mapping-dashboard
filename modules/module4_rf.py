...
def render_leaderboard(board: list):
    ...
    for i, e in enumerate(sboard[:10]):
        ...
        <div class="lb-row">
            ...
            <div class="lb-event">{e.get('test_events','')}</div>
            <div class="lb-bar-wrap">
                <div class="lb-bar-bg">
                    <div class="lb-bar-fill" style="width:{bw}%"></div>
                </div>
            </div>
            <div class="lb-score">{e['accuracy']*100:.1f}%</div>
        </div>
        """
    st.markdown(f"""
    <div class="lb-wrap">
...
def render_feature_importance(importance: dict):
    ...
    fig.update_layout(
        ...
        xaxis=dict(showgrid=True, gridcolor="#f1f5f9", ticksuffix="%",
                   tickfont=dict(color="#94a3b8", size=10), range=[0, max(values)*1.2]),
...
def render_confusion_matrix(cm: list):
    ...
    fig.update_layout(
        ...
        title=dict(text="Confusion Matrix", font=dict(size=12, color="#475569"), x=0.5),
    )
    st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
